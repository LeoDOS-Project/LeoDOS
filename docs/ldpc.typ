...
Reed-Solomon (covered in `coding.typ`) corrects _byte_ errors and
...
A codeword $bold(c)$ of $n$ bits is valid if and only if:

$ H dot bold(c)^T = bold(0) quad (mod 2) $
...
Row 0 says: $c_0 xor c_1 xor c_3 xor c_4 = 0$. If the received
...
#table(
  columns: (0.8fr, 0.6fr, 0.8fr, 0.8fr, 0.8fr),
  stroke: none,
  inset: 5pt,
...
defined as a grid of $M times M$ sub-matrix blocks, where $M$ is the
...
- *Zero block* ($H_Z$): an $M times M$ all-zeros matrix --- this
  check doesn't connect to any bit in this column.
- *Identity block* ($H_I$): the $M times M$ identity matrix ---
  check $i$ connects to bit $i$ in this column.
- *Permutation block* ($H_(P_k)$): an $M times M$ matrix defined by
...
#align(center)[
  #text(size: 9pt)[
    *Rate 1/2* (3 #sym.times 5 blocks): \
    #table(
      columns: 5,
      stroke: 0.5pt,
      inset: 4pt,
      align: center,
      $H_Z$, $H_Z$, $H_I$, $H_Z$, $H_I + H_(P_0)$,
      $H_I$, $H_I$, $H_Z$, $H_I$, $H_(P_1) + H_(P_2) + H_(P_3)$,
      $H_I$, $H_(P_4) + H_(P_5)$, $H_Z$, $H_(P_6) + H_(P_7)$, $H_I$,
    )
  ]
]
...
Each permutation $pi_k$ maps row position $i$ (in $0 dots M-1$) to
a column position within the same block. It is defined by two lookup
tables, $theta$ and $phi$, from the CCSDS standard:

$ pi_k (i) = M/4 dot ((theta_k + floor(4i slash M)) mod 4) + (phi_(floor(4i slash M), k) + i) mod M/4 $

The formula splits $M$ into 4 quadrants. $theta_k$ rotates which
...
For $M = 512$ and $k = 0$: $theta_0 = 3$, $phi_(0,0) = 16$. So:

$ pi_0 (0) = 128 dot ((3 + 0) mod 4) + (16 + 0) mod 128 = 384 + 16 = 400 $

$ pi_0 (200) = 128 dot ((3 + 1) mod 4) + (16 + 200) mod 128 = 0 + 88 = 88 $

The CCSDS standard defines 26 permutations ($k = 0 dots 25$). How
many are used depends on the rate: rate~1/2 uses $k = 0 dots 7$,
rate~2/3 uses $k = 0 dots 13$, rate~4/5 uses $k = 0 dots 25$.
...
$ "parity" = "info" times P quad (mod 2) $

The codeword is then $["info" | "parity"]$, with $k$ info bits
followed by $n - k$ parity bits.

=== Circulant Structure

The matrix $P$ has dimensions $k times (n - k)$. Storing it
directly would require $1024 times 1024 = 131072$ bits for
rate~1/2. Instead, $P$ is decomposed into _circulant blocks_ of
size $b times b$, where $b$ is the circulant size ($b = M/4$).
...
$ P_"compact" = k/b "rows" times (n-k)/64 " u64 values per row" $
...
+ For each rotation offset $o = 0, 1, dots, b - 1$:
  + For each compact row $r = 0, 1, dots, k/b - 1$:
    + The info bit at position $r dot b + o$: if it is 1, XOR the
      compact generator row $r$ into the parity accumulator.
  + Left-rotate each parity block by 1 bit (simulating the
...
To verify a received codeword, we check $H dot bold(c)^T = bold(0)$.
...
$ "punctured"[i] = xor.big_("all other connections in row 2, pos" space i) "codeword"["bit"] $

We compute this for every $i in 0 dots M - 1$, giving us all $M$
punctured bits.
...
+ *Variable $arrow.r$ Check:* each bit tells each of its checks
  "here's my current belief, excluding what you told me last time."
+ *Check $arrow.r$ Variable:* each check combines messages from all
...
*Why $M = 512$ for rate~1/2?* The base matrix has $"nb" = 5$
columns. The transmitted codeword has $("nb" - 1) dot M = 4M$ bits.
...
#table(
  ...
  [*Permutations*], [$pi_k$ with $theta$/$phi$ lookup tables],
...
we use the test vector `info[i] = i` for $i = 0 dots 127$:
...
valid codeword, all $2 times 512 = 1024$ check equations evaluate
to zero.
