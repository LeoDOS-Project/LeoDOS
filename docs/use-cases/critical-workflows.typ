...
+ *Registration.* Ground uploads a workflow definition: AOI bounding
...
+ *Baseline acquisition.* On the first qualifying pass, collectors
  acquire data and store it as the reference. No alert is generated.

+ *Monitoring loop.* On each subsequent pass over the AOI:
  - *Collect:* acquire fresh sensor data.
  - *Map:* compare against baseline (per-pixel or per-patch), flag
    anomalies exceeding threshold.
  - *Reduce:* aggregate flagged pixels into a compact alert descriptor
    (location, magnitude, confidence).
  - *Decision:* if anomalies found, route alert packet to the next LOS
    node for priority downlink.

+ *Baseline update.* Optionally, the baseline can be refreshed
  periodically (e.g. every _N_ passes) to account for seasonal
  changes, or the ground can upload a new one.

+ *Deregistration.* Ground sends a cancel command, or the workflow
...
- *Map:* interferometric phase subtraction against a stored master
  image (differential InSAR). For each pixel, compute displacement:
  $ Delta d = (Delta phi dot lambda) / (4 pi) $
- *Reduce:* count pixels where $|Delta d| >$ threshold (e.g. 5 mm). If
...
- *Map:* for each pixel, compute brightness temperature. Flag pixels
  where $T >$ contextual threshold (accounting for solar heating, land
  cover). Compare against a running background model.
- *Reduce:* cluster adjacent hot pixels. Filter false positives (sun
...
*Pipeline:*
- *Collect:* multispectral image over protected forest AOI.
- *Map:* compute NDVI per pixel:
  $ "NDVI" = ("NIR" - "Red") / ("NIR" + "Red") $
  Compare against baseline NDVI map. Flag pixels where $Delta$NDVI
  exceeds deforestation threshold (e.g. drop of > 0.3).
- *Reduce:* cluster changed pixels, compute area in hectares. Filter
...
*Pipeline:*
- *Collect:* SAR image over monitored shipping lanes or offshore
  platforms.
- *Map:* detect dark spots on ocean surface (oil dampens capillary
  waves, reducing SAR backscatter). Apply adaptive threshold relative
  to surrounding sea state.
- *Reduce:* classify dark spots by shape (elongated = likely spill,
  circular = natural slick/low-wind zone). Filter by area (> minimum
  spill size). Generate alert with spill extent estimate.

*Alert payload:* ~2 KB (centroid, estimated area km#super[2], elongation
...
*Pipeline:*
- *Collect:* SAR image over flood-risk AOI.
- *Map:* classify pixels as water/non-water using backscatter threshold
  (water is specularly reflective #sym.arrow.r dark in SAR). Compare
  against pre-flood baseline to identify newly inundated areas.
- *Reduce:* compute total flooded area, identify affected
  infrastructure (by overlaying with a stored vector map of
  roads/buildings). Generate alert.

*Alert payload:* ~5 KB (flood extent polygon, area km#super[2], list of
...
*Pipeline:*
- *Collect:* SAR strip over monitored Arctic corridor.
- *Map:* edge detection on ice boundaries. Compare against previous
  pass to detect fracture propagation.
- *Reduce:* if fracture rate exceeds threshold or new leads (open water
...
- *Map:* change detection against a baseline backscatter image. Flag
  ...
  reflector geometry).
- *Reduce:* filter detections by time of day (nighttime = elevated
...
#table(
  ...
  inset: 8pt,
  align: left,
  table.header([*Phase*], [*Operation*], [*Output*]),
...
#sym.arrow.r alert packet (KB). This is precisely what makes onboard
...
+ Use the existing Router to forward alerts toward the LOS node.
+ Use SRSPP's reliable transport to guarantee delivery.
+ Tag alerts with priority to enable preemptive scheduling over routine
  telemetry.
...
+ *Startup*: registers workflows from a table loaded via CFE Table
  ...
  replay).
+ *Collection trigger*: the app periodically reads its GPS position
...
+ *Processing*: the map and reduce phases execute onboard. For
  ...
  processing pipeline.
+ *Alert routing*: if anomalies are detected, the app sends an alert
  packet via SRSPP through the ISL router toward the nearest LOS
  node for ground downlink.
+ *State persistence*: baselines and workflow metadata are stored in
...
#table(
  ...
  [Tailings dam], [Synthetic raster], [Generate SLC pair with
    displacement phase screen. Master image stored as baseline.
    Inject increasing displacement over successive passes.],
  [Wildfire], [Parametric], [Thermal hotspot injection is simple
    (coordinates + temperature). Full raster adds little value
    since the detection algorithm is a threshold.],
  [Deforestation], [Real data replay], [Sentinel-2 NDVI time series
    over known deforestation events (e.g. Brazilian Amazon).
    Provides realistic seasonal variation for false-positive
    testing.],
  [Oil spill], [Synthetic raster], [Generate SAR backscatter image
    with inserted dark spot of configurable shape and area.
    Background sea clutter from a statistical model.],
  [Flood], [Real data replay], [Sentinel-1 SAR over documented
    flood events (e.g. 2024 Valencia floods). Pre-flood / post-flood
    image pair for change detection validation.],
  [Volcanic], [Synthetic raster], [Same as tailings dam — different
    AOI, larger displacement magnitudes (cm-scale).],
  [Sea ice], [Real data replay], [Sentinel-1 SAR over Arctic
    corridors during known breakup events. Edge detection algorithms
    benefit from real ice texture.],
  [Anti-poaching], [Synthetic raster], [Generate SAR backscatter
    with injected point targets (vehicles) on access roads.
    Vary time-of-day and location to test nighttime filtering
    and reserve boundary logic.],
)
