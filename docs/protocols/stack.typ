...
#let active-fill = rgb("#d4e8d4")
#let inactive-fill = luma(245)

#let sublayer(body, width: 100%, active: true) = {
  rect(
    width: width,
    stroke: 0.5pt + if active { luma(120) } else { luma(190) },
    inset: 3pt,
    fill: if active { active-fill } else { inactive-fill },
    text(weight: "bold", size: 7.5pt, fill: if active { black } else { luma(140) })[#body]
  )
}
...
#let alt-active(..items) = {
  grid(
    columns: (1fr,) * items.pos().len(),
    column-gutter: 2pt,
    ..items.pos().map(c => {
      let (name, is-active) = c
      sublayer(name, width: 100%, active: is-active)
    })
  )
}
...
#layer("Application")[
  #alt-active(([SpaceCoMP], true), ([ColonyOS], true))
]
#v(-5pt)
#layer("Transport")[
  #alt-active(([SRSPP], true), ([CFDP], true))
]
#v(-5pt)
#layer("Network")[
  #group("cFE Headers")[
    #alt-active(([TM], true), ([TC], true))
  ]
  #v(-4pt)
  #group("Routing")[
    #alt-active(([ISL Router], true), ([PassThrough], true), ([Gossip], false))
  ]
]
#v(-5pt)
#layer("Data Link")[
  #group("Packet Protocol")[
    #sublayer([SPP], active: true)
  ]
  #v(-4pt)
  #group("Transfer Frame Protocols")[
    #alt-active(([TM], true), ([TC], true), ([AOS], false), ([Proximity-1], false), ([USLP], false))
  ]
  #v(-4pt)
  #group("Security")[
    #sublayer([SDLS], active: true)
  ]
  #v(-4pt)
  #group("Reliability")[
    #sublayer([COP-1], active: true)
  ]
]
#v(-5pt)
#layer("Coding")[
  #group("Randomization")[
    #sublayer([Randomization], active: true)
  ]
  #v(-4pt)
  #group("Forward Error Correction")[
    #alt-active(([RS(255,223)], true), ([LDPC (AR4JA)], true), ([Convolutional], false))
  ]
  #v(-4pt)
  #group("Framing")[
    #alt-active(([ASM / CADU], true), ([CLTU], true))
  ]
  #v(-4pt)
  #group("Data Compression (payload)")[
    #alt-active(([Rice], true), ([DWT], true), ([Hyperspectral], true))
  ]
]
#v(-5pt)
#layer("Physical")[
  #group("Modulation")[
    #alt-active(([BPSK], true), ([QPSK], true), ([OQPSK], false), ([8PSK], false), ([GMSK], false))
  ]
  #v(-4pt)
  #group("Hardware")[
    #alt-active(([UART], true), ([SPI], true), ([I2C], true), ([CAN], true), ([GPIO], true), ([UDP], true), ([TCP], true))
  ]
]
...
#grid(
  ...
  column-gutter: 8pt,
  rect(width: 12pt, height: 10pt, fill: active-fill, stroke: 0.5pt + luma(120)),
  text(size: 8pt)[Implemented],
  [],
)
