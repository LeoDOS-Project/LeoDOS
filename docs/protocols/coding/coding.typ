...
$ p_0 = d_0 dot alpha^2 + d_1 dot alpha + d_2 $

and another:

$ p_1 = d_0 dot alpha^4 + d_1 dot alpha^2 + d_2 $
...
fails: $200 times 200 = 40000$, which doesn't fit. If intermediate
...
symbols #sym.times 1 byte each) falls apart.
...
$ alpha^0 = 1, quad alpha^1 = 2, quad alpha^2 = 4, quad dots, quad alpha^(254) = 142, quad alpha^(255) = 1 #text[ (wraps around)] $
...
exponents: $alpha^a times alpha^b = alpha^(a+b)$.
...
Take the 223 data bytes, say $[d_0, d_1, d_2, dots]$. We treat
them as coefficients of a polynomial:

$ d_0 dot x^(222) + d_1 dot x^(221) + d_2 dot x^(220) + dots.c + d_(222) $
...
$ g(x) = (x - 2^(112))(x - 2^(113)) dots.c (x - 2^(143)) $
...
$ C(2^(112)) = Q(2^(112)) dot underbrace(g(2^(112)), = 0) = 0 $
...
For a single error, the ratio $S_1 slash S_0$ directly reveals the
...
positions $p_1, p_2, dots, p_t$, define $X_k = 2^(254 - p_k)$ for
each error and:

$ sigma(x) = (1 - X_1 x)(1 - X_2 x) dots.c (1 - X_t x) = 1 + sigma_1 x + sigma_2 x^2 + dots.c + sigma_t x^t $
...
+ Start with $sigma(x) = 1$ (no errors assumed).
+ For each syndrome $S_n$ ($n = 0, 1, dots, 31$), compute a
  _discrepancy_:
  $ Delta_n = S_n + sigma_1 dot S_(n-1) + sigma_2 dot S_(n-2) + dots.c + sigma_l dot S_(n-l) $
  where $l$ is the current number of estimated errors.
+ If $Delta_n = 0$, the current $sigma$ already predicts $S_n$
  correctly --- no update needed.
+ If $Delta_n eq.not 0$, the current $sigma$ is wrong. Update it
  using a correction term scaled by $Delta_n$. If this increases
  the estimated error count ($2l lt.eq n$), also update $l$.
+ After all 32 syndromes, $l$ is the number of errors and $sigma(x)$
...
+ For each $m = 0, 1, dots, 254$, compute $sigma(2^m)$ using the
  GF($2^8$) lookup tables.
+ If $sigma(2^m) = 0$, then $X = 2^m$ is a root, meaning there is
  an error at position $p = (m + 254) mod 255$.
+ Collect all positions where $sigma$ evaluates to zero.
...
$ Omega(x) = S(x) dot sigma(x) mod x^(32) $

where $S(x) = S_0 + S_1 x + S_2 x^2 + dots.c + S_(31) x^(31)$ is
...
$ sigma'(x) = sigma_1 + sigma_3 x^2 + sigma_5 x^4 + dots.c $

The error magnitude at position $k$ is then:

$ e_k = X_k^(1 - 112) dot Omega(X_k^(-1)) / sigma'(X_k^(-1)) $
...
+ *Berlekamp-Massey* finds $l > 16$: the syndrome equations imply
  more errors than the code can handle.
+ *Chien search* finds fewer roots than $l$: the error locator
  polynomial has no valid solution within the 255 positions.
+ *Verification* after correction: the recomputed syndromes are
...
each syndrome gives 1 equation. 32 syndromes $arrow.r$ at most 16
solvable errors.
...
A burst of $16 dot I$ corrupted bytes gets spread across $I$
...
#table(
  ...
  [*Field polynomial*], [$x^8 + x^7 + x^2 + x + 1$ (#raw("0x187"))],
...
$ D(x) dot x^(32) = 72 x^(254) + 69 x^(253) + 76 x^(252) + 76 x^(251) + 79 x^(250) $

The generator $g(x) = (x - 2^(112))(x - 2^(113)) dots.c (x - 2^(143))$
is a fixed degree-32 polynomial.

Divide $D(x) dot x^(32)$ by $g(x)$. The remainder $R(x)$ has 32
...
The received 255 bytes $[r_0, r_1, dots, r_(254)]$ form a
polynomial:

$ R(x) = r_0 dot x^(254) + r_1 dot x^(253) + dots.c + r_(254) $
...
$ S_0 &= r_0 dot (2^(112))^(254) + r_1 dot (2^(112))^(253) + r_2 dot (2^(112))^(252) + dots.c + r_(254) $

Each term like $(2^(112))^(254)$ simplifies to $2^((112 dot 254) mod 255) = 2^(93)$,
which is just another byte from the EXP table. Plugging in the
received bytes ($r_0 = 72$, $r_1 = 69$, $r_2 = 255$, ...):

$ S_0 = 72 dot 2^(93) + 69 dot 2^(236) + 255 dot 2^(124) + dots.c = 219 $
...
$ S_j = r_0 dot 2^((112+j) dot 254 mod 255) + r_1 dot 2^((112+j) dot 253 mod 255) + dots.c + r_(254) $

for $j = 0, 1, dots, 31$. The same bytes, but different
...
$C(x) = Q(x) dot g(x)$, where
$g(x) = (x - 2^(112))(x - 2^(113)) dots.c (x - 2^(143))$.
To compute $S_0$, we evaluate $C(x)$ at $x = 2^(112)$:

$ S_0 = C(2^(112)) = Q(2^(112)) dot g(2^(112)) $

Expanding $g(2^(112))$:

$ g(2^(112)) = (2^(112) - 2^(112))(2^(112) - 2^(113)) dots.c (2^(112) - 2^(143)) $

The first factor is $2^(112) minus 2^(112) = 0$ (any value XOR'd
...
$ S_0 = 0, quad S_1 = 0, quad S_2 = 0, quad dots, quad S_(31) = 0 $

After the corruption at byte\ 2, the syndromes become non-zero:

$ S_0 = 219, quad S_1 = 232, quad S_2 = 29, quad S_3 = 145, quad S_4 = 67, quad dots $
...
$sigma(x) arrow.l 1 + 219 x$, $l arrow.l 1$.

*Iteration $n = 1$:* $Delta_1 = S_1 plus.circle sigma_1 dot S_0
= 232 plus.circle "mul"(219, 219) = 232 plus.circle 101 = 141$.
Non-zero, so update $sigma$. Since $l$ doesn't increase ($2 dot 1 > 1$),
only the coefficients change: $sigma(x) arrow.l 1 + 81 x$.

*Iteration $n = 2$:* $Delta_2 = S_2 plus.circle sigma_1 dot S_1
= 29 plus.circle "mul"(81, 232) = 29 plus.circle 29 = 0$.
...
$ sigma(alpha^0) &= 1 plus.circle "mul"(81, 1) &&= 1 plus.circle 81 &&= 80 \
  sigma(alpha^1) &= 1 plus.circle "mul"(81, 2) &&= 1 plus.circle 162 &&= 163 \
  sigma(alpha^2) &= 1 plus.circle "mul"(81, 4) &&= 1 plus.circle 195 &&= 194 \
  sigma(alpha^3) &= 1 plus.circle "mul"(81, 8) &&= 1 plus.circle 1 &&= 0 quad arrow.l "root!" $

$sigma(alpha^3) = 0$, so $alpha^3$ is a root. The error position is
$(3 + 254) mod 255 = 2$. That is byte~2 --- exactly where the `L`
was corrupted.

=== 5. Error Magnitude (Forney)

The Forney algorithm computes the error evaluator polynomial
$Omega(x) = S(x) dot sigma(x) mod x^(32)$, which starts:

$ Omega_0 = S_0 dot sigma_0 = 219 dot 1 = 219 $
$ Omega_1 = S_1 dot sigma_0 plus.circle S_0 dot sigma_1 = 232 plus.circle "mul"(219, 81) = 232 plus.circle 232 = 0 $
...
$ e = X^(1 - 112) dot Omega(X^(-1)) / sigma'(X^(-1)) = alpha^(78) dot 219 / 81 = 19 dot 196 = 179 $

where $X^(1 - 112) = alpha^(252 dot 144 mod 255) = alpha^(78) = 19$ adjusts for
the first consecutive root, and $219 / 81 = 196$ in GF($2^8$).

Correction is XOR:

$ 255 xor 179 = 76 quad ("the original" `L`!) $
