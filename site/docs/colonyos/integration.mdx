...
- **Multi-node jobs** — a single [SpaceCoMP](/spacecomp/overview) job requires multiple satellites working together ([collectors, mappers, reducer](/spacecomp/roles)). ColonyOS assigns a process to a single executor — there is no built-in concept of a group of cooperating executors sharing one job.
...
- **Embedded execution** — satellites run `no_std` Rust on [cFS](/cfs/overview) with no HTTP, no Go runtime, and no general-purpose OS. Satellites cannot run a ColonyOS executor directly.
