...
However, the ground coordinator design remains the right approach even with a GEO relay. The relay adds ~250 ms round-trip latency (LEO→GEO→ground→GEO→LEO), which is fine for uploading a job plan but not for coordinating each step of a multi-satellite computation in real time. Satellites should still execute autonomously over ISL after receiving the plan, rather than continuously synchronizing with the ground through the relay.
