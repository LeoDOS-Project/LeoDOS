...
SRSPP builds on top of CCSDS Space Packets (SPP). It reuses fields from the SPP primary header rather than adding its own:
...
<Packet title="DATA Packet (13 + N bytes)">
  ...
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    ...
    <Field name="Packet Type" bits="2 bits" desc="0 = DATA" />
    <Field name="Spare" bits="4 bits" desc="Reserved (0)" />
  </Header>
  <Payload label="Application Payload (variable)" color="#88aa88" />
</Packet>

<Packet title="ACK Packet (17 bytes, fixed)">
  <Header title="SPP Primary Header (6 bytes)" color="#eef3ff" border="#6688bb">
    <Field name="Version" bits="3 bits" desc="Packet version (0)" />
    <Field name="Type" bits="1 bit" desc="1 = Telecommand" />
    <Field name="Sec Hdr Flag" bits="1 bit" desc="1 = Present" />
    <Field name="APID" bits="11 bits" desc="Destination (0–2047)" highlight="#dce8f8" />
    <Field name="Seq Flags" bits="2 bits" desc="Unsegmented (0b11)" />
    <Field name="Seq Count" bits="14 bits" desc="Sequence (0–16383)" highlight="#ddf0dd" />
    <Field name="Data Length" bits="16 bits" desc="Remaining length − 1" />
  </Header>
  <Header title="cFE Secondary Header (2 bytes)" color="#f5f0ff" border="#8866bb">
  ...
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    <Field name="Source Address" bits="16 bits" desc="Sender (orb, sat)" />
    <Field name="Version" bits="2 bits" desc="Protocol version (0)" />
    <Field name="Packet Type" bits="2 bits" desc="1 = ACK" />
    <Field name="Spare" bits="4 bits" desc="Reserved (0)" />
  </Header>
...
- **Cumulative ACK** (ACK only) — the highest sequence number for which all packets up to and including that number have been received. The sender can free all packets up to this number.
- **Selective Bitmap** (ACK only) — a 16-bit bitmap where each bit represents a packet beyond the cumulative ACK. Bit 0 = cumulative+1, bit 1 = cumulative+2, etc. Set bits indicate received out-of-order packets that do not need retransmission.
