...
- **Packet lost** — the sender's retransmission timer expires. It resends the packet and restarts the timer. If the packet is lost again, it retries up to `max_retransmits` times (default 3). After that, the packet is declared lost and the application is notified.
