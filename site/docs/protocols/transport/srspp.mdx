...
SRSPP uses a sliding window: the sender can have multiple packets in flight simultaneously, and the receiver acknowledges them with cumulative + selective ACKs.
...
export const Pkt = ({seq, color, border, label}) => (
  <div style={{
    ...
    <div>{seq}</div>
    {label && <div style={{fontSize: '0.8em', color: '#666'}}>{label}</div>}
  </div>
);

<div style={{margin: '16px 0'}}>
<div style={{fontSize: '0.85em', fontWeight: 'bold', marginBottom: 4}}>Sender's view (window size = 4):</div>
<div style={{display: 'flex', gap: 3, alignItems: 'center'}}>
  ...
  <div style={{borderLeft: '3px solid #cc4444', height: 40, margin: '0 2px'}} />
  <Pkt seq="3" color="#c8daf8" border="2px solid #6688bb" label="sent" />
  <Pkt seq="4" color="#c8daf8" border="2px solid #6688bb" label="sent" />
  <Pkt seq="5" color="#f5e0b0" border="2px solid #bba050" label="wait" />
  <Pkt seq="6" color="#f5e0b0" border="2px solid #bba050" label="wait" />
  <div style={{borderLeft: '3px solid #cc4444', height: 40, margin: '0 2px'}} />
  <Pkt seq="7" label="next" />
  <Pkt seq="8" label="next" />
</div>
<div style={{display: 'flex', gap: 16, fontSize: '0.75em', marginTop: 6, color: '#666'}}>
  <span>← ACKed (freed)</span>
  <span style={{color: '#cc4444'}}>|</span>
  <span>← Window (in flight + waiting) →</span>
  <span style={{color: '#cc4444'}}>|</span>
  <span>Not yet queued →</span>
</div>
</div>

<div style={{display: 'flex', gap: 16, fontSize: '0.8em', marginBottom: 16, flexWrap: 'wrap'}}>
  ...
  <div style={{display: 'flex', alignItems: 'center', gap: 4}}><div style={{width: 14, height: 14, background: '#c8daf8', border: '1px solid #6688bb', borderRadius: 3}} /> Sent, awaiting ACK</div>
  <div style={{display: 'flex', alignItems: 'center', gap: 4}}><div style={{width: 14, height: 14, background: '#f5e0b0', border: '1px solid #bba050', borderRadius: 3}} /> Queued, waiting for window space</div>
</div>

When the receiver sends an ACK with cumulative=2, packets 0–2 are freed and the window slides forward, allowing packets 7–8 to enter the window. If packet 3 is lost, the receiver can still acknowledge packet 4 via the selective bitmap — the sender knows not to retransmit 4 and only retries 3 when its timer expires.
