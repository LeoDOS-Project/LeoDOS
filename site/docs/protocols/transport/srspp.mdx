...
<Packet title="DATA Packet (13 + N bytes)">
  ...
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    ...
    <Field name="Packet Type" bits="8 bits" desc="0x00 = DATA" />
  </Header>
...
<Packet title="ACK Packet (17 bytes, fixed)">
  ...
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    <Field name="Source Address" bits="16 bits" desc="Sender (orb, sat)" />
    <Field name="Packet Type" bits="8 bits" desc="0x01 = ACK" />
  </Header>
...
The packet has four header layers. SRSPP reuses the **SPP primary header** for destination (APID), sequencing (Seq Count), and segmentation (Seq Flags). The **cFE secondary header** provides a function code and integrity checksum. The **ISL routing header** carries the target satellite address for multi-hop forwarding. SRSPP adds only the **source address** and **packet type** (DATA or ACK).
