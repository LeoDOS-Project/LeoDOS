...
<Packet title="SRSPP DATA Packet">
  <Header title="SPP Primary Header (6 bytes)" color="#eef3ff" border="#6688bb">
    <Field name="Version" bits="3 bits" />
    <Field name="Type" bits="1 bit" desc="0 = TM, 1 = TC" />
    <Field name="Sec Hdr" bits="1 bit" desc="Secondary header present" />
    <Field name="APID" bits="11 bits" desc="Destination address" highlight="#dce8f8" />
    <Field name="Seq Flags" bits="2 bits" desc="FIRST / CONT / LAST / UNSEG" highlight="#ddf0dd" />
    <Field name="Seq Count" bits="14 bits" desc="Packet sequence number" highlight="#ddf0dd" />
    <Field name="Length" bits="16 bits" desc="Remaining packet length − 1" />
  </Header>
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    <Field name="Source" bits="16 bits" desc="Sender spacecraft address" />
    <Field name="Type" bits="2 bits" desc="0 = DATA, 1 = ACK, 2 = EOS" />
    <Field name="Version" bits="2 bits" desc="Protocol version (0)" />
    <Field name="Spare" bits="4 bits" />
  </Header>
  <Payload label="Payload (variable)" color="#88aa88" />
</Packet>

### SRSPP ACK Packet

<Packet title="SRSPP ACK Packet">
  <Header title="SPP Primary Header (6 bytes)" color="#eef3ff" border="#6688bb">
    <Field name="APID" bits="11 bits" desc="Destination address" highlight="#dce8f8" />
    <Field name="Seq Count" bits="14 bits" desc="Packet sequence number" highlight="#ddf0dd" />
    <Field name="..." bits="" desc="(other fields same as above)" />
  </Header>
  <Header title="SRSPP Header (3 bytes)" color="#fff8ee" border="#bba050">
    <Field name="Source" bits="16 bits" desc="Sender spacecraft address" />
    <Field name="Type" bits="2 bits" desc="1 = ACK" />
    <Field name="..." bits="" />
  </Header>
  <Header title="ACK Payload (4 bytes)" color="#f0fff0" border="#88aa88">
    <Field name="Cum ACK" bits="16 bits" desc="Highest in-order seq received" />
    <Field name="Bitmap" bits="16 bits" desc="Selective ACK for out-of-order packets" />
  </Header>
</Packet>

SRSPP reuses the SPP primary header rather than duplicating fields. The **APID** identifies the destination, the **Seq Count** provides ordering and loss detection, and the **Seq Flags** handle message segmentation (splitting messages larger than the MTU into FIRST/CONTINUATION/LAST segments). SRSPP adds only the source address and packet type.
