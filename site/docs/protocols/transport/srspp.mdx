...
export const Field = ({name, color, border, width}) => (
  <div style={{
...
<div style={{fontSize: '0.75em', fontWeight: 'bold', marginBottom: 4}}>SPP Primary Header (6 bytes) — fields used by SRSPP:</div>
<div style={{display: 'flex', gap: 2, flexWrap: 'wrap'}}>
  <Field name="Version (3b)" />
  <Field name="Type (1b)" />
  <Field name="Sec Hdr (1b)" />
  <Field name="APID (11b)" color="#c8daf8" border="1px solid #6688bb" />
  <Field name="Seq Flags (2b)" color="#d4e8d4" border="1px solid #88aa88" />
  <Field name="Seq Count (14b)" color="#d4e8d4" border="1px solid #88aa88" />
  <Field name="Length (16b)" />
</div>
<div style={{fontSize: '0.7em', color: '#666', marginTop: 4}}>
  <span style={{color: '#6688bb'}}>■</span> APID = destination address &nbsp;
  <span style={{color: '#88aa88'}}>■</span> Seq Flags = segmentation (FIRST/CONT/LAST/UNSEG), Seq Count = packet sequence number for ordering and loss detection
</div>
</div>

SRSPP adds a 3-byte header after the SPP header containing the source address, packet type (DATA/ACK/EOS), and protocol version. The sequence count from SPP serves as the packet number — no additional sequence field is needed.
