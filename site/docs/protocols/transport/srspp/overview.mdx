...
**How streams are identified.** Each stream is identified by the pair of source address (from the SRSPP header) and destination APID (from the [SPP](/protocols/datalink/packet/spp) header). Only one sender per spacecraft is supported for each destination APID.
...
- [Packet Structure](packet-structure) — header layout, field descriptions
- [Flow Control](flow-control) — sliding window, send buffer
- [Reassembly](reassembly) — receiver backends: Fast, Lite, Packed
