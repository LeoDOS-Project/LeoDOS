...
export const Phase = ({label, color, border, children}) => (
  <div style={{
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
  }}>
    <div style={{fontSize: '0.7em', fontWeight: 'bold', color: '#666', textTransform: 'uppercase', letterSpacing: 1}}>{label}</div>
...
export const Sat = ({name, color, border}) => (
  <div style={{
    border: `2px solid ${border}`,
    borderRadius: 5, padding: '4px 12px',
    background: color, fontSize: '0.8em', fontWeight: 'bold',
    textAlign: 'center', minWidth: 80,
  }}>{name}</div>
);

export const Flow = ({label}) => (
  <div style={{display: 'flex', alignItems: 'center', padding: '0 8px', fontSize: '0.7em', color: '#666'}}>
    <div>→</div>
    <div style={{padding: '0 4px', whiteSpace: 'nowrap'}}>{label}</div>
    <div>→</div>
  </div>
);
...
<div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, minWidth: 600}}>
  <Phase label="Collect">
    <Sat name="Collector A" color="#d4e8d4" border="#88aa88" />
    <Sat name="Collector B" color="#d4e8d4" border="#88aa88" />
  </Phase>
  <Flow label="data" />
  <Phase label="Map">
    <Sat name="Mapper A" color="#d4e8d4" border="#88aa88" />
    <Sat name="Mapper B" color="#d4e8d4" border="#88aa88" />
  </Phase>
  <Flow label="results" />
  <Phase label="Reduce">
    <Sat name="Reducer" color="#d4e8d4" border="#88aa88" />
  </Phase>
  <Flow label="output" />
  <Phase label="">
    <Sat name="Ground" color="#e8eef8" border="#6688bb" />
  </Phase>
</div>
<div style={{textAlign: 'center', fontSize: '0.75em', color: '#888', marginTop: 6}}>
  The <span style={{color: '#bba050', fontWeight: 'bold'}}>coordinator</span> plans the job and assigns roles. Data flows left to right through the pipeline.
</div>
</div>
