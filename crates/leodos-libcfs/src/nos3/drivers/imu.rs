...
use crate::ffi;
use crate::nos3::buses::can::check;
use crate::nos3::buses::can::Can;
use crate::nos3::buses::can::CanError;
...
pub fn command(device: &mut Can, cmd_code: u8) -> Result<(), CanError> {
    check(unsafe { ffi::GENERIC_IMU_CommandDevice(&mut device.inner, cmd_code) })
}

/// Requests housekeeping telemetry from the IMU.
pub fn request_hk(device: &mut Can) -> Result<ImuHk, CanError> {
    let mut raw = ffi::GENERIC_IMU_Device_HK_tlm_t::default();
    check(unsafe { ffi::GENERIC_IMU_RequestHK(&mut device.inner, &mut raw) })?;
    Ok(ImuHk {
...
pub fn request_axis(device: &mut Can, cmd_code: u8) -> Result<AxisData, CanError> {
    let mut raw = ffi::GENERIC_IMU_Device_Axis_Data_t::default();
    check(unsafe { ffi::GENERIC_IMU_RequestAxis(&mut device.inner, &mut raw, cmd_code) })?;
    Ok(AxisData {
...
pub fn request_data(device: &mut Can) -> Result<ImuData, CanError> {
    let mut raw = ffi::GENERIC_IMU_Device_Data_tlm_t::default();
    check(unsafe { ffi::GENERIC_IMU_RequestData(&mut device.inner, &mut raw) })?;
    Ok(ImuData {
        x: AxisData {
