...
mod bitmasks {
    ...
    pub const fn get_bits(bitmap: u16, mask: u16) -> u16 {
        (bitmap & mask) >> mask.trailing_zeros()
    }

    /// Returns `bitmap` with the bits specified by `mask` set to `value`.
    pub const fn set_bits(bitmap: u16, mask: u16, value: u16) -> u16 {
        (bitmap & !mask) | (value << mask.trailing_zeros())
    }
}
use bitmasks::*;
...
impl Apid {
    ...
    pub fn new(id: u16) -> Result<Self, BuildError> {
        if id > Self::MAX {
...
impl SpacePacket {
    /// Creates a new `SpacePacketBuilder` for constructing packets.
    pub fn builder() -> builder::SpacePacketBuilder<Vacant, Vacant, Vacant, Vacant, Vacant, Vacant>
    {
        builder::SpacePacketBuilder::new()
    }
    ...
    pub fn new<'a>(
        ...
        let packet_bytes = &mut buffer[..required_len];
        let packet = Self::mut_from_bytes(packet_bytes)
        ...
        packet.set_sequence_count(packet_sequence_count);
        packet.set_data_field_len(data_field_len);
        packet.set_secondary_header_flag(SecondaryHeaderFlag::Absent);
        packet.set_sequence_flag(SequenceFlag::Unsegmented);
...
impl PrimaryHeader {
    ...
    pub fn version(&self) -> PacketVersion {
        ...
        PacketVersion(get_bits(ident, PACKET_VERSION_MASK) as u8)
    }
    ...
    pub fn set_version(&mut self, version: PacketVersion) {
        ...
        self.packet_version_and_id
            .set(set_bits(ident, PACKET_VERSION_MASK, version.0 as u16));
    }

    /// Returns the `PacketType` (Telemetry or Telecommand).
    pub fn packet_type(&self) -> PacketType {
        if get_bits(self.packet_version_and_id.get(), PACKET_TYPE_MASK) == 0 {
    ...
    pub fn set_packet_type(&mut self, packet_type: PacketType) {
        let ident = self.packet_version_and_id.get();
        self.packet_version_and_id
            .set(set_bits(ident, PACKET_TYPE_MASK, packet_type as u16));
    }

    /// Returns the `SecondaryHeader` flag (Present or Absent).
    pub fn secondary_header_flag(&self) -> SecondaryHeaderFlag {
        if get_bits(self.packet_version_and_id.get(), SEC_HDR_MASK) == 0 {
    ...
    pub fn set_secondary_header_flag(&mut self, flag: SecondaryHeaderFlag) {
        let ident = self.packet_version_and_id.get();
        self.packet_version_and_id
            .set(set_bits(ident, SEC_HDR_MASK, flag as u16));
    }
    ...
    pub fn sequence_flag(&self) -> SequenceFlag {
        match get_bits(self.packet_sequence_control.get(), SEQ_FLAG_MASK) {
    ...
    pub fn set_sequence_flag(&mut self, flag: SequenceFlag) {
        let seq = self.packet_sequence_control.get();
        self.packet_sequence_control
            .set(set_bits(seq, SEQ_FLAG_MASK, flag as u16));
    }

    /// Returns the 14-bit packet sequence count.
    pub fn sequence_count(&self) -> PacketSequenceCount {
        let seq = self.packet_sequence_control.get();
        PacketSequenceCount(get_bits(seq, SEQ_COUNT_MASK))
    }

    /// Sets the 14-bit `SequenceCount`.
    pub fn set_sequence_count(&mut self, count: PacketSequenceCount) {
        let seq = self.packet_sequence_control.get();
        self.packet_sequence_control
            .set(set_bits(seq, SEQ_COUNT_MASK, count.0));
    }
...
impl From<u16> for PacketSequenceCount {
...
mod kani_harness {
    ...
    fn packet_construction() {
        ...
        let sequence_count = PacketSequenceCount::any();
        let data_field_len: u16 = kani::any();

        let result = SpacePacket::new(
            &mut bytes,
            apid,
            packet_type,
            sequence_count,
...
mod tests {
    ...
    fn build_and_set_data() {
        ...
        let data_len = size_of::<TelemetryData>() as u16;
    ...
    fn deserialize_trivial_packet() {
        ...
        assert_eq!(packet.sequence_count(), PacketSequenceCount::from(0));
    ...
    fn roundtrip_header_fields() {
        ...
        for _ in 0..10_000 {
            ...
            let sequence_count = PacketSequenceCount::from(rng.next_u32() as u16);
            let data_field_len: u16 = 1;
